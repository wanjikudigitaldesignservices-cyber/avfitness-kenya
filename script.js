document.addEventListener('DOMContentLoaded',()=>{
    ...
    document.getElementById('bookingForm').addEventListener('submit',e=>{
        e.preventDefault();const d=Object.fromEntries(new FormData(e.target));d.id=Date.now();d.submittedAt=new Date().toISOString();d.status='New Lead';
        const all=JSON.parse(localStorage.getItem('avfitness_leads')||'[]');all.push(d);localStorage.setItem('avfitness_leads',JSON.stringify(all));
        e.target.reset();showToast('APPLICATION RECEIVED. WE WILL CONTACT YOU SOON.');
    });
});
function bookProg(p){
    const s=document.getElementById('interestSelect');
    Array.from(s.options).forEach(o=>{if(o.value.includes(p))s.value=o.value});
